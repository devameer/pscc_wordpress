...
while (have_posts()) {
    the_post();
    $hero_data = function_exists('get_field') ? (get_field('publications_hero') ?: []) : [];
...
if (!$publications_query->have_posts()) {
    echo '<main class="container mx-auto px-4 py-16 text-center text-slate-600">' . esc_html__('No publications found. Please add some via the dashboard.', 'beit') . '</main>';
    get_footer();
    return;
}
...
<main class="bg-gray-50 text-slate-900">
    <section class="container mx-auto px-3 py-8 sm:px-4 sm:py-10 md:px-6 md:py-16">
        ...
        $pagination_links = paginate_links(
            [
                'total' => max(1, (int) $publications_query->max_num_pages),
                'current' => max(1, $paged),
                'type' => 'array',
            ]
        );
