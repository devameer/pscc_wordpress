    ...
    <div class="container mx-auto px-4 md:px-6 relative z-10">
        <div class="mb-8 md:mb-12 space-y-3 md:space-y-4 text-center" data-aos="fade-up">
            <?php if (!empty($facts['title'])): ?>
                    <h2 class="text-3xl font-normal md:text-5xl"><?php echo wp_kses_post($facts['title']); ?></h2>
            <?php endif; ?>
            <?php if (!empty($facts['subtitle'])): ?>
                    <p class="text-base text-white  md:text-lg font-normal max-w-sm mx-auto">
                        <?php echo esc_html($facts['subtitle']); ?></p>
            <?php endif; ?>
        </div>

        <!-- Years Tabs -->
        <div class="mb-8 md:mb-12 flex flex-wrap items-center justify-center gap-2 md:gap-3" data-aos="fade-up"
            data-aos-delay="100">
        ...
        <?php foreach ($years as $year_index => $year):
            ...
            ?>
                <div class="facts-year-content grid gap-6 md:grid-cols-2 lg:grid-cols-4 <?php echo !$is_active ? 'hidden' : ''; ?>"
                    data-year="<?php echo esc_attr($year_id); ?>">
                    <?php
                    $fact_index = 0;
                    foreach ($year_items as $fact):
                        ...
                        ?>
                            <div class=" bg-white/30 backdrop-blur-sm p-6 md:p-8 text-center shadow-lg h-52 md:h-60 flex flex-col justify-center items-center gap-3 md:gap-4"
                                data-aos="flip-up" data-aos-delay="<?php echo esc_attr($fact_delay); ?>">
                                <?php if ($value): ?>
                                        <div class="text-3xl md:text-4xl lg:text-6xl font-extrabold" data-counter
                                            data-target="<?php echo esc_attr($value); ?>">0</div>
                                <?php endif; ?>
                                <?php if ($label): ?>
                                        <div class="mt-2 md:mt-3 text-xs md:text-sm font-bold uppercase tracking-widest text-white/70">
